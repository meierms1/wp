...
const WeatherView = ({ data }) => {
  const [selectedId, setSelectedId]     = useState('atmosphere');
  ...
  // Reset all sections open on topic change
  ...
  return (
    <div>
      {/* ── Horizon bar – color accent that changes per topic ── */}
      ...
      >
        {data.map(t => {
          const tc       = COLORS[t.color] || COLORS.blue;
