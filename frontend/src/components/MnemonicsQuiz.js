...
/** Accept the answer if it matches any slash-separated variant, case-insensitively */
function isCorrect(input, word) {
  const norm = (s) => s.trim().toLowerCase().replace(/\s+/g, ' ');
  const val = norm(input);
  if (!val) return false;
  const variants = word.split(/\s*\/\s*/).map(norm);
  return variants.includes(val) || norm(word) === val;
}
...
const MnemonicsQuiz = ({ data, onBack }) => {
  ...
  // Focus first input when a new mnemonic loads
  useEffect(() => {
    if (!checked) {
  ...
  const handleCheck = useCallback(() => {
    ...
    let correct = 0;
    mnemonic.letters.forEach((l, i) => {
      if (isCorrect(answers[i] ?? '', l.word)) correct++;
    });
          ...
          <div className={`bg-gradient-to-br ${c.card} rounded-2xl border p-5 mb-4 backdrop-blur-sm`}>
            <div className="flex flex-wrap gap-2 mb-3">
              {mnemonic.acronym.split('').map((ch, i) => (
                <span key={i} className={`w-11 h-11 flex items-center justify-center rounded-xl font-black text-xl border ${c.letter}`}>
                  {ch}
                </span>
              ))}
            </div>
            <h3 className="text-white font-bold text-lg leading-tight">{mnemonic.title}</h3>
            <p className={`text-sm mt-0.5 ${c.text} opacity-75`}>{mnemonic.description}</p>
          </div>

          {/* Letter rows */}
          <div className="space-y-2 mb-5">
            {mnemonic.letters.map((l, i) => {
              const correct = checked ? isCorrect(answers[i] ?? '', l.word) : null;
              return (
                <motion.div
                  key={i}
                ...
                >
                      ...
                      <div>
                        ...
                        {!correct && (
                          <p className="text-white/50 text-xs mt-0.5">
                            ✓ <span className="text-white/75">{l.word}</span>
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  {checked && (
                    <motion.span
                      ...
                      className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${correct ? 'bg-green-500/30' : 'bg-red-500/30'}`}
                    >
                      {correct
          ...
          <div className="flex gap-3">
            {!checked ? (
              <motion.button
                ...
                onClick={handleCheck}
                className={`flex-1 py-3 rounded-xl font-semibold text-white border transition-colors ${c.card} ${c.letter.split(' ').find(s => s.startsWith('border-'))} hover:brightness-125`}
              >
                Check Answers
              </motion.button>
            ) : (
